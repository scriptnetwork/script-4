...
func (msgr *Messenger) processLoop(ctx context.Context) {
	defer func() {
		...
		allPeers := msgr.peerTable.GetAllPeers() // should clean up all peers, including edge nodes
	...
	for {
		...
		case pid := <-msgr.newPeers:
			...
			if int(msgr.peerTable.GetTotalNumPeers()) >= viper.GetInt(common.CfgP2PMaxNumPeers) { // only account for blockchain nodes
				msgr.host.Network().ClosePeer(pid)
				continue
			}
...
func (msgr *Messenger) maintainSufficientConnections(ctx context.Context) {
	diff := viper.GetInt(common.CfgP2PMinNumPeers) - int(msgr.peerTable.GetTotalNumPeers()) // only account for blockchain nodes
	if diff > 0 {
...
func (msgr *Messenger) printStats() {
	...
	ret := "Received bytes:"
	for k := byte(0); k < byte(common.ChannelIDMax); k++ {
