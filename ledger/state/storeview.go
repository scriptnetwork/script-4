...
log.Debug("XXXXXXXXXXXXXXX ledger/state/storeview.go::GetAccount");
...
func (sv *StoreView) SubScriptBalance(addr common.Address, amount *big.Int) {
	if amount.Sign() == 0 {
		return
	}
	account := sv.GetAccount(addr)
	...
	account.Balance = account.Balance.NoNil()
	account.Balance.SCPTWei.Sub(account.Balance.SCPTWei, amount)
	sv.SetAccount(addr, account)
...
func (sv *StoreView) AddScriptBalance(addr common.Address, amount *big.Int) {
	if amount.Sign() == 0 {
		return
	}
	account := sv.GetOrCreateAccount(addr)
	account.Balance = account.Balance.NoNil()
	account.Balance.SCPTWei.Add(account.Balance.SCPTWei, amount)
	sv.SetAccount(addr, account)
...
func (sv *StoreView) GetScriptStake(addr common.Address) *big.Int {
	...
	gcp := sv.GetLightningCandidatePool()
...
func (sv *StoreView) Suicide(addr common.Address) bool {
	...
	sv.addBalanceChange(&types.BalanceChange{
		...
		TokenType:  1,
		IsNegative: true,
		Delta:      new(big.Int).Set(account.Balance.SPAYWei),
	})
