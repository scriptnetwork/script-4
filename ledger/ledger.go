...
func (ledger *Ledger) GetLightningCandidatePool(blockHash common.Hash) (*core.LightningCandidatePool, error) {
	...
	for {
		logger.Debugf("Ledger.GetLightningCandidatePool, block.height = %v", block.Height)
...
func (ledger *Ledger) handleEliteEdgeNodeStakeReturns(view *st.StoreView) {
	...
	for _, returnedStakeWithHolder := range returnedStakesWithHolders {
		...
		returnedCoins := types.Coins{ // Important: Elite edge nodes deposit/withdraw SPAY stake, NOT Script
			SCPTWei: types.Zero,
			SPAYWei: returnedStake.Amount,
		}
		sourceAccount.Balance = sourceAccount.Balance.Plus(returnedCoins)
		view.SetAccount(sourceAddress, sourceAccount)
