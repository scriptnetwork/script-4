...
func TestChainID(t *testing.T) {
	...
	chainIDStrPrivatenet := "scriptnet"
...
func TestSendTxSignable2(t *testing.T) {
	...
	scriptWei := new(big.Int).Mul(new(big.Int).SetUint64(10), ten18)
	...
	sendTx := &SendTx{
		...
		Inputs: []TxInput{
			TxInput{
				Address:  senderAddr,
				Coins:    Coins{SCPTWei: scriptWei, SPAYWei: new(big.Int).Add(spayWei, feeInSPAYWei)},
				Sequence: 2,
			},
		},
		Outputs: []TxOutput{
			TxOutput{
				...
				Coins:   Coins{SCPTWei: scriptWei, SPAYWei: spayWei},
			},
		},
	}
	signBytes := sendTx.SignBytes(chainID)
