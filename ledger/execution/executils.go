...
func sanityCheckForFee(fee types.Coins, blockHeight uint64) (minimumFee *big.Int, success bool) {
	fee = fee.NoNil()
	minimumFee = types.GetMinimumTransactionFeeSPAYWei(blockHeight)
	success = (fee.SCPTWei.Cmp(types.Zero) == 0 && fee.SPAYWei.Cmp(minimumFee) >= 0)
...
func sanityCheckForSendTxFee(fee types.Coins, numAccountsAffected uint64, blockHeight uint64) (minimumFee *big.Int, success bool) {
	fee = fee.NoNil()
	minimumFee = types.GetSendTxMinimumTransactionFeeSPAYWei(numAccountsAffected, blockHeight)
	success = (fee.SCPTWei.Cmp(types.Zero) == 0 && fee.SPAYWei.Cmp(minimumFee) >= 0)
