...
func (c *transferScript) Run(evm *EVM, input []byte, callerAddr common.Address, contract *Contract) ([]byte, error) {
	...
	scriptWeiAmount := new(big.Int).SetBytes(getData(input, 20, 32))
	if !CanTransferScript(evm.StateDB, callerAddr, scriptWeiAmount) {
		return common.Bytes{}, ErrInsufficientScriptBlance
	}

	// send Script from the contract to the specified recipient
	TransferScript(evm.StateDB, callerAddr, recipient, scriptWeiAmount)
...
// RequiredGas returns the gas required to execute the pre-compiled contract.
func (c *stakeToLightning) RequiredGas(input []byte, blockHeight uint64) uint64 {
	return params.StakeToLightningGas
}

func (c *stakeToLightning) Run(evm *EVM, input []byte, callerAddr common.Address, contract *Contract) ([]byte, error) {
	lightningSummary := getData(input, 0, 229)
	scriptWeiAmount := new(big.Int).SetBytes(getData(input, 229, 32))

	ok := StakeToLightning(evm.StateDB, callerAddr, lightningSummary, scriptWeiAmount)
	if !ok {
...
// unstakeFromLightning unstake Script from Lightning node
type unstakeFromLightning struct {
}

// RequiredGas returns the gas required to execute the pre-compiled contract.
func (c *unstakeFromLightning) RequiredGas(input []byte, blockHeight uint64) uint64 {
	return params.UnstakeFromLightningGas
}

func (c *unstakeFromLightning) Run(evm *EVM, input []byte, callerAddr common.Address, contract *Contract) ([]byte, error) {
	lightningAddr := common.BytesToAddress(input)
	ok := UnstakeFromLightning(evm.StateDB, callerAddr, lightningAddr)
	if !ok {
		return common.Bytes{}, ErrInvalidStakeOperation
	}
	return common.Bytes{}, nil
