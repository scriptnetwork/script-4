...
// CanTransferScript checks whether there are enough funds in the address' account to make a Script transfer.
func CanTransferScript(db StateDB, addr common.Address, amount *big.Int) bool {
	return db.GetScriptBalance(addr).Cmp(amount) >= 0
}
...
// TransferScript subtracts the given amount of Script from sender and adds amount to recipient using the given Db
func TransferScript(db StateDB, sender, recipient common.Address, amount *big.Int) {
	db.SubScriptBalance(sender, amount)
	db.AddScriptBalance(recipient, amount)
}
...
func CheckBLSSummary(summary []byte) bool {
	lightningAddr, blsPubkey, blsPop, holderSig, ok := parseBLSSummary(summary)
	if !ok {
		return false
	}

	return checkBlsSummary(blsPubkey, blsPop, holderSig, lightningAddr)
}

func checkBlsSummary(blsPubkey *bls.PublicKey, blsPop *bls.Signature, holderSig *crypto.Signature, lightningAddr common.Address) bool {
	...
	if !holderSig.Verify(blsPop.ToBytes(), lightningAddr) {
		return false
	}
...
func (evm *EVM) Call(caller ContractRef, addr common.Address, input []byte, gas uint64, value *big.Int, scriptValue *big.Int) (ret []byte, leftOverGas uint64, err error) {
	...
	contract := NewContract(caller, to, value, scriptValue, gas)
...
func (evm *EVM) CallCode(caller ContractRef, addr common.Address, input []byte, gas uint64, value *big.Int, scriptValue *big.Int) (ret []byte, leftOverGas uint64, err error) {
	if evm.vmConfig.NoRecursion && evm.depth > 0 {
		return nil, gas, nil
	}
...
func (evm *EVM) create(caller ContractRef, codeAndHash *codeAndHash, gas uint64, value *big.Int, scriptValue *big.Int, address common.Address) ([]byte, common.Address, uint64, error) {
	...
	// EVM. The contract is a scoped environment for this execution context
	// only.
	contract := NewContract(caller, AccountRef(address), value, scriptValue, gas)
...
// Create creates a new contract using code as deployment code.
func (evm *EVM) Create(caller ContractRef, code []byte, gas uint64, value *big.Int, scriptValue *big.Int) (ret []byte, contractAddr common.Address, leftOverGas uint64, err error) {
	contractAddr = crypto.CreateAddress(caller.Address(), evm.StateDB.GetNonce(caller.Address()))
	return evm.create(caller, &codeAndHash{code: code}, gas, value, scriptValue, contractAddr)
}
...
func (evm *EVM) Create2(caller ContractRef, code []byte, gas uint64, endowment *big.Int, scriptEndowment *big.Int, salt *big.Int) (ret []byte, contractAddr common.Address, leftOverGas uint64, err error) {
	codeAndHash := &codeAndHash{code: code}
	contractAddr = crypto.CreateAddress2(caller.Address(), common.BigToHash(salt), codeAndHash.Hash().Bytes())
	return evm.create(caller, codeAndHash, gas, endowment, scriptEndowment, contractAddr)
}
