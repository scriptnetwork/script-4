...
// Test case for lightning stake deposit, withdrawal, and return
func TestLightningStakeUpdate(t *testing.T) {
	assert := assert.New(t)
	...
	depositStakeTx := &types.DepositStakeTxV2{
		...
		Source: types.TxInput{
			Address: depositSourcePrivAcc.Address,
			Coins: types.Coins{
				SCPTWei: new(big.Int).Set(core.MinLightningStakeDeposit),
				SPAYWei: new(big.Int).SetUint64(0),
			},
			Sequence: 1,
		},
		...
		Purpose: core.StakeForLightning,
	}
	signBytes := depositStakeTx.SignBytes(es.chainID)
	depositStakeTx.Source.Signature = depositSourcePrivAcc.Sign(signBytes)

	// ----------- Lightning's first deposit must include valid BLS Pubkey/Pop -------- //
	_, res := es.executor.ExecuteTx(depositStakeTx)
	...
	// ----------- BLS Pubkey/Pop in existing lightning's deposit should be ignored -------- //
	depositStakeTx = &types.DepositStakeTxV2{
		Fee: types.NewCoins(0, txFee),
		Source: types.TxInput{
			Address: depositSourcePrivAcc.Address,
			Coins: types.Coins{
				SCPTWei: new(big.Int).Mul(new(big.Int).SetUint64(2), core.MinLightningStakeDeposit),
				SPAYWei: new(big.Int).SetUint64(0),
			},
			Sequence: 1,
		},
	...
	// ----------- Lightning's deposit can omit BLS Pubkey/Pop -------- //
	depositStakeTx = &types.DepositStakeTxV2{
		Fee: types.NewCoins(0, txFee),
		Source: types.TxInput{
			Address: depositSourcePrivAcc.Address,
			Coins: types.Coins{
				SCPTWei: new(big.Int).Mul(new(big.Int).SetUint64(3), core.MinLightningStakeDeposit),
				SPAYWei: new(big.Int).SetUint64(0),
			},
			Sequence: 2,
		},
	...
	// Validate lightning pool
	gcp := st.NewStoreView(b0.Height, b0.StateHash, db).GetLightningCandidatePool()
	log.Infof("gcp for block #0: %v", gcp)
	assert.Equal(0, gcp.Len())

	gcp = st.NewStoreView(b1.Height, b1.StateHash, db).GetLightningCandidatePool()
	log.Infof("gcp for block #1: %v", gcp)
	assert.Equal(1, gcp.Len())
	assert.Equal(0, gcp.SortedLightnings[0].TotalStake().Cmp(
		new(big.Int).Mul(new(big.Int).SetUint64(1), core.MinLightningStakeDeposit)))

	// Lightning's BLS Pubkey in record should not be changed after first deposit.
	gcp2 := st.NewStoreView(b2.Height, b2.StateHash, db).GetLightningCandidatePool()
	log.Infof("gcp for block #2: %v", gcp2)
	assert.Equal(1, gcp2.Len())
	assert.Equal(0, gcp2.SortedLightnings[0].TotalStake().Cmp(
		new(big.Int).Mul(new(big.Int).SetUint64(3), core.MinLightningStakeDeposit)))
	assert.Equal(gcp.SortedLightnings[0].Pubkey, gcp2.SortedLightnings[0].Pubkey)

	// Lightning's BLS Pubkey in record should not be changed after first deposit.
	gcp3 := st.NewStoreView(b3.Height, b3.StateHash, db).GetLightningCandidatePool()
	log.Infof("gcp for block #3: %v", gcp3)
	assert.Equal(1, gcp3.Len())
	assert.Equal(0, gcp3.SortedLightnings[0].TotalStake().Cmp(
		new(big.Int).Mul(new(big.Int).SetUint64(6), core.MinLightningStakeDeposit)))
	assert.Equal(gcp.SortedLightnings[0].Pubkey, gcp2.SortedLightnings[0].Pubkey)
	...
	gcp4 := st.NewStoreView(b11.Height, b11.StateHash, db).GetLightningCandidatePool()
	...
	assert.Equal(0, gcp4.SortedLightnings[0].TotalStake().Cmp(core.MinLightningStakeDeposit))
	assert.Equal(gcp.SortedLightnings[0].Pubkey, gcp2.SortedLightnings[0].Pubkey)
	...
	// The 1st deposit(1*minimal) was from holder to holder , 2nd deposit(2*minimal) and 3rd deposit
	// (3*minimal)was from source to holder.
	assert.Equal(0, returnedCoins.SCPTWei.Cmp(new(big.Int).Mul(new(big.Int).SetUint64(5), core.MinLightningStakeDeposit)))
	assert.True(returnedCoins.SPAYWei.Cmp(core.Zero) == 0)
	log.Infof("Returned coins: %v", returnedCoins)
}
