...
func TestValidators2(t *testing.T) {
	assert := assert.New(t)
	...
	*/
	...
	va1AddrStr := "0x111"
	va1 := common.HexToAddress(va1AddrStr)
	//va1 := NewAddressSet(va1AddrStr, va1Stake)

	va2AddrStr := "0x222"
	va2 := common.HexToAddress(va2AddrStr)
	//va2 := NewValidator(va2AddrStr, va2Stake)

	va3AddrStr := "0x333"
	va3 := common.HexToAddress(va3AddrStr)
	//va3 := NewValidator(va3AddrStr, va3Stake)

	va4AddrStr := "0x444"
	va4 := common.HexToAddress(va4AddrStr)
...
func TestValidators(t *testing.T) {
	...
	*/
	validators.Add(sourceAddr1)
	validators.Add(sourceAddr2)
	validators.Add(sourceAddr3)
	validators.Add(sourceAddr4)
	assert.True(len(validators.ToSortedSlice()) == 4)
	//assert.True(validators.ToSortedSlice()[0].TotalStake().Cmp(new(big.Int).Mul(new(big.Int).SetUint64(13200), MinValidatorStakeDeposit)) == 0)
	checkAndPrintAllSortedCandidates(t, assert, validators)
	checkAndPrintTopCandidates(t, assert, validators, 3)
	...
	*/
	...
	assert.True(len(*validators) == 4)
	//	assert.True(len(validators.SortedCandidates) == 4)
	checkAndPrintAllSortedCandidates(t, assert, validators)
	checkAndPrintTopCandidates(t, assert, validators, 3)
	...
	assert.True(len(*validators) == 4) // holderAddr1's stake not returned yet, it should still be in the candidate list
	assert.True(validators.ToSortedSlice()[3] == holderAddr2)
	//assert.True(validators.ToSortedSlice()[3].TotalStake().Cmp(Zero) == 0) // All stakes are withdrawn
	checkAndPrintAllSortedCandidates(t, assert, validators)
	checkAndPrintTopCandidates(t, assert, validators, 3)
...
func TestValidatorSetUniqueSortedOrder(t *testing.T) {
	...
	*/
	...
	vals := validators.ToSortedSlice()
	vcpJson1, _ := json.MarshalIndent(vals, "", "  ")
	fmt.Printf("validators after the 1st sort: %v\n\n", string(vcpJson1))

	vals2 := validators.ToSortedSlice()
	vcpJson2, _ := json.MarshalIndent(vals2, "", "  ")
	fmt.Printf("validators after the 2nd sort: %v\n\n", string(vcpJson2))
	...
	assert.Equal(vals[0], holderAddr4)
	assert.Equal(vals[1], holderAddr3)
	assert.Equal(vals[2], holderAddr2)
	assert.Equal(vals[3], holderAddr1)
	assert.Equal(vals[4], holderAddr6)
	assert.Equal(vals[5], holderAddr5)

	assert.Equal(vcpJson1, vcpJson2)
	//ssert.Equal(vcpJson2, vcpJson3)
	//assert.Equal(vcpJson3, vcpJson4)
}

// ------------------------- Utilities -------------------------

func checkAndPrintAllSortedCandidates(t *testing.T, assert *assert.Assertions, validators *AddressSet) {
	...
	for _, sh := range validators.ToSortedSlice() {
		holder := sh
		//	stake := sh.TotalStake()
		log.Infof("Holder: %v", holder)
		//assert.True(prevStake.Cmp(stake) >= 0) // Should be sorted in the descending order
		//prevStake = stake
	}
}

func checkAndPrintTopCandidates(t *testing.T, assert *assert.Assertions, validators *AddressSet, numCandidates int) {
	log.Infof(fmt.Sprintf("------ Top %v Candidates ------", numCandidates))
	topCands := validators.ToSortedSlice() //GetTopStakeHolders(3)
	if len(topCands) > numCandidates {
		topCands = topCands[:numCandidates]
	}
	//prevStake := new(big.Int).Mul(new(big.Int).SetUint64(99999999999), MinValidatorStakeDeposit) // some big number
