...
func TestDedup(t *testing.T) {
	...
	vs := NewAddressSet()
	vs.Add(common.HexToAddress("A1"))
	vs.Add(common.HexToAddress("A3"))
	vs.Add(common.HexToAddress("A4"))
...
func TestCommitCertificate(t *testing.T) {
	...
	*/
	...
	va1 := priv1.PublicKey().Address()

	priv2, _, _ := crypto.GenerateKeyPair()
	va2 := priv2.PublicKey().Address()

	priv3, _, _ := crypto.GenerateKeyPair()
	va3 := priv3.PublicKey().Address()

	priv4, _, _ := crypto.GenerateKeyPair()
	va4 := priv4.PublicKey().Address()
	...
	vote1 := Vote{ID: va1, Block: blockHash, Height: 1}
	vote1.Sign(priv1)
	vote2 := Vote{ID: va2, Block: blockHash, Height: 1}
	vote2.Sign(priv2)
	vote3 := Vote{ID: va3, Block: blockHash, Height: 1}
	vote3.Sign(priv3)
	vote4 := Vote{ID: va4, Block: blockHash, Height: 1}
	vote4.Sign(priv4)
	...
	voteSet := NewVoteSet()
	voteSet.AddVote(Vote{ID: va1})
	voteSet.AddVote(Vote{ID: va2})
	voteSet.AddVote(Vote{ID: va1})
	cc = CommitCertificate{Votes: invalidVoteSet, BlockHash: blockHash}
	assert.False(cc.IsValid(&vs))
	...
	voteSet.AddVote(Vote{ID: va1})
	voteSet.AddVote(Vote{ID: va2})
	voteSet.AddVote(Vote{ID: va3, Block: common.HexToHash("0x11")})
	cc = CommitCertificate{Votes: invalidVoteSet, BlockHash: blockHash}
	assert.False(cc.IsValid(&vs))
}
