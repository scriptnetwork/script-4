...
func NewConsensusEngine(privateKey *crypto.PrivateKey, db store.Store, chain *blockchain.Chain, dispatcher *dispatcher.Dispatcher, validatorManager core.ValidatorManager) *ConsensusEngine {
	...
	e.lightning = NewLightningEngine(e, blsKey)
...
func (e *ConsensusEngine) Stop() {
	...
	if e.lightningTimer != nil {
		e.lightningTimer.Stop()
	}
}
		...
		for {
			...
			case <-e.lightningTimer.C:
				...
				if v != nil {
					...
					e.broadcastLightningVote(v)
				}
				e.lightning.StartNewRound()
...
func (e *ConsensusEngine) validateBlock(block *core.Block, parent *core.ExtendedBlock) result.Result {
	...
	// Validate Lightning Votes.
	// We allow checkpoint blocs to have nil lightning votes.
	if block.LightningVotes != nil && block.Height >= common.HeightEnableScript2 && common.IsCheckPointHeight(block.Height) {
	...
	if block.EliteEdgeNodeVotes != nil && block.Height >= common.HeightEnableScript3 && common.IsCheckPointHeight(block.Height) {
		// Voted block must exist.
		padding := uint64(20)
		if e.chain.Root().Height+padding*uint64(common.CheckpointInterval) < block.Height {
...
func (e *ConsensusEngine) handleVote(vote core.Vote) (endEpoch bool) {
	...
	if vote.Epoch >= e.GetEpoch() {
		...
		if nextValidators.HasMajority(currentEpochVotes) {
			...
			e.logger.Debugf("========================================================================== New Epoch %v", nextEpoch)
			e.logger.WithFields(log.Fields{
				"e.epoch":          e.GetEpoch,
				"nextEpoch":        nextEpoch,
...
func (e *ConsensusEngine) broadcastLightningVote(vote *core.AggregatedVotes) {
	payload, err := rlp.EncodeToBytes(vote)
	if err != nil {
		e.logger.WithFields(log.Fields{"lightning vote": vote}).Error("Failed to encode vote")
		return
	}
	voteMsg := dispatcher.DataResponse{
		ChannelID: common.ChannelIDLightning,
		Payload:   payload,
	}
	e.dispatcher.SendData([]string{}, voteMsg)
}
...
func (e *ConsensusEngine) finalizeBlock(block *core.ExtendedBlock) error {
	...
	// Lightnings and Elite Edge Nodes to vote for checkpoint blocks.
	if common.IsCheckPointHeight(block.Height) {
		e.lightning.StartNewBlock(block.Hash())
...
func (e *ConsensusEngine) createProposal(shouldIncludeValidatorUpdateTxs bool) (core.Proposal, error) {
	...
	if block.Height >= common.HeightEnableScript3 && common.IsCheckPointHeight(block.Height) {
...
func (e *ConsensusEngine) resetLightningTimer() {
	if e.lightningTimer != nil {
		e.lightningTimer.Stop()
	}
	e.lightningTimer = time.NewTicker(time.Duration(viper.GetInt(common.CfgLightningRoundLength)) * time.Second)
}
